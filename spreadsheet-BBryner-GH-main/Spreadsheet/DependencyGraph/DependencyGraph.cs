...
// Methods and Constructor implemented by Brenden Bryner (September 11, 2025)
...
public class DependencyGraph
{
    ...
    /// <summary>
    /// Dictionary for tracking dependents
    /// </summary>
    private Dictionary<string, HashSet<string>> _dependents;
    /// <summary>
    /// dictionary for tracking dependees 
    /// </summary>
    private Dictionary<string, HashSet<string>> _dependees;
    /// <summary>
    /// tracks the size of the dependency graph (how many ordered pairs)
    /// </summary>
    private int _size;
    ...
    /// </summary>
    /// <para>Should be in constant time</para>
    /// <param name="nodeName"> The name of the node.</param>
    /// <returns> true if the node has dependents. </returns>
    public bool HasDependents(string nodeName)
    {
        if (_dependents.ContainsKey(nodeName))
            return _dependents[nodeName].Count > 0;
        else
            return false;
    }
    ...
    public bool HasDependees(string nodeName)
    {
        if (_dependees.ContainsKey(nodeName))
            return _dependees[nodeName].Count > 0;
        else
            return false;
    }
    ...
    {
        // check for the node and make a new hashset with the dependents
        if (_dependents.ContainsKey(nodeName))
            return new  HashSet<string>(_dependents[nodeName]);
        else
            // return empty one if there arent dependents
    ...
    {
        // same process as get dependents but with dependees
        if (_dependees.ContainsKey(nodeName))
            return new   HashSet<string>(_dependees[nodeName]);
        else
            return new HashSet<string>();
    }
    ...
    {
        // make sure that the dependee is in the dependent dictionary
        if (!_dependents.ContainsKey(dependee))
            _dependents[dependee] = new HashSet<string>();
        
        // Make sure that the dependent is in the dependee dictionary
        if (!_dependees.ContainsKey(dependent))
            _dependees[dependent] = new HashSet<string>();
        
        // Add it if it does not exist and update size
        if (_dependents[dependee].Add(dependent))
        {
    ...
    ///   </para>
    /// <para>Shoudl be in constant time</para>
    /// </summary>
    /// <param name="dependee"> The name of the node that must be evaluated first</param>
    /// <param name="dependent"> The name of the node that cannot be evaluated until after dependee</param>
    public void RemoveDependency(string dependee, string dependent)
    {
        // make sure if the dependency exists and remove it from the dependents dictionary
        if (_dependents.ContainsKey(dependee) && _dependents[dependee].Remove(dependent))
        {
            // Remove from dependees dictionary and decrement size
            if (_dependees.ContainsKey(dependent))
            {
    ...
    {
        // get rid of all existing dependencies where the nodeName is the dependee
        if (_dependents.ContainsKey(nodeName))
        {
            // make a copy so the iteration doesn't make errors
            foreach (var dependent in _dependents[nodeName].ToList())
            {
    ...
    {
        // remove all the dependencies where the nodename is the dependent
        if (_dependees.ContainsKey(nodeName))
        {
            //make copy 
            foreach (var dependee in _dependees[nodeName].ToList())
            {
